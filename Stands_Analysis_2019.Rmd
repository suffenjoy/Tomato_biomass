...
ggplot(subset(df_PRI_phys,Date>"2019-07-01"), aes(x = CHL, y = PRIm)) + geom_point(aes(color = ID)) + scale_color_d3() + theme_bw() + geom_smooth(method = "lm", formula = y~x) + xlab("Chlorophyll Content (ug cm-2)") + ylab("PRI")
...
#Soil Moisture data 
```{r}
unique(gssb_filt$Date)
unique(gssb_filt$Time)

summary(gssb_filt$VWC) #there are some values close to 1000
gssb_filt_vwc <- subset(gssb_filt, VWC<800 & VWC > 0)

hist(gssb$VWC)
gssb_vwc <- subset(gssb, VWC < 800 & VWC>0)

VWC_daily <- aggregate(gssb_vwc$VWC, list("Date"=gssb_vwc$Date, "ID"=gssb_vwc$ID, "BLOCK"=gssb_vwc$BLOCK, "TREATMENT"=gssb_vwc$TREATMENT), function(x){mean(x,na.rm=TRUE)})
names(VWC_daily)[5] <- "VWC_all"
ggplot(VWC_daily, aes(x = Date, y = VWC_all)) + geom_point(aes(color = ID)) + theme_bw() + scale_color_d3() + facet_wrap(~TREATMENT)
VWC_daily <- aggregate(gssb_filt_vwc$VWC, list("Date"=gssb_filt_vwc$Date, "ID" = gssb_filt_vwc$ID, "BLOCK" = gssb_filt_vwc$BLOCK, "TREATMENT" = gssb_filt_vwc$TREATMENT), function(x){mean(x,na.rm = TRUE)})
dim(VWC_daily)
head(VWC_daily)
names(VWC_daily)[5] <- "VWC_diurnal"
ggplot(VWC_daily, aes(x = Date, y = VWC_diurnal)) + geom_point(aes(color = ID)) + theme_bw() + scale_color_d3() + facet_wrap(~TREATMENT)
weather_daily_all <- aggregate(weather[,c('Radiation','AirTemp','VPD')], list("Date"= weather$Date), function(x){mean(x,na.rm = TRUE)})
