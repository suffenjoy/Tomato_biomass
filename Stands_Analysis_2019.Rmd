...
###PRImin-PRI0 (follow Troy's paper)
```{r}
...
#calculate PRIdelta
PRIdelta.ma_season <- merge(PRImin.ma_season, PRI0.ma_season, by = c("Date","ID"))
PRIdelta.ma_season <- PRIdelta.ma_season %>% mutate(PRImin_PRI0.ma = PRImin.ma - PRI0.ma)
ggplot(PRIdelta.ma_season, aes(x = Date, y = PRImin_PRI0.ma, color = ID)) + geom_point()  + scale_color_d3() + theme_bw() + ylab("PRImin - PRI0") + ylim(c(-0.1, 0.025)) + ggtitle("Seasonal Change of PRImin - PRI0") + geom_smooth(se = FALSE)
```
...
names(temp_split[[1]]) #PRIm.ma 44, NDVIm 43, PRI0.ma 48, PRIdelta.ma 50
#PRIm daily average
temp_ls <- lapply(temp_split, function(x){PRIm_dayave(x, 11, 15, "2019-05-11", "2019-09-04")})
temp_ls2 <- lapply(temp_split, function(x){DayAve(x, 11, 15, "2019-05-11", "2019-09-04", 50)})
for(i in 1:length(temp_ls)){
  temp_ls[[i]]$ID = names(temp_ls)[i]
}
